...
def _margen_del_rango(Factura, DetalleFactura, Producto, desde, hasta):
    ...
    SUPUESTOS DE DATOS (FactuFácil + Schiro):
    - Producto.costo se guarda CON IVA incluido.
    - DetalleFactura.costo_unitario se guarda CON IVA (snapshot al vender).
    - DetalleFactura.subtotal y precio_unitario se guardan SIN IVA.
    Para comparar venta vs costo hay que llevar ambos a NETO: dividir el costo
    por (1 + iva/100) usando la alícuota REAL de cada producto/línea.

    Lógica alineada 1:1 con el reporte /api/margen_productos (estadisticas.py).
    """
    try:
        # FIX 21/05/2026: DetalleFactura.costo_unitario también se guarda CON IVA
        # (es snapshot de Producto.costo al momento de la venta). Hay que dividir
        # AMBOS por (1+iva/100) para llevarlos a neto y poder compararlos con
        # DetalleFactura.subtotal (que viene sin IVA).
        # Antes solo el fallback se dividía → el margen quedaba ~3% en vez de ~20%.
        # Lógica alineada 1:1 con /api/margen_productos en estadisticas.py.
        iva_detalle = func.coalesce(func.nullif(DetalleFactura.porcentaje_iva, 0), 21)
        iva_producto = func.coalesce(func.nullif(Producto.iva, 0), 21)
        costo_expr = func.coalesce(
            func.nullif(DetalleFactura.costo_unitario, 0) / (1 + iva_detalle / 100),
            Producto.costo / (1 + iva_producto / 100)
        )
