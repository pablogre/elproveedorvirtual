...
def _nombre_vendedor(db, vendedor_id):
    """Nombre del vendedor para títulos. None si no aplica o no existe la tabla."""
    if not vendedor_id:
        return None
    try:
        row = db.session.execute(
            text("SELECT nombre FROM vendedor WHERE id = :id"),
            {'id': vendedor_id}).mappings().first()
        return row['nombre'] if row else f'Vendedor #{vendedor_id}'
    except Exception:
        return f'Vendedor #{vendedor_id}'


def _deuda_clientes_data(db, vendedor_id=None):
    ...
    filtro_vend = "AND c.vendedor_id = :vid" if vendedor_id else ""
    params_saldos = {'vid': vendedor_id} if vendedor_id else {}
    saldos = db.session.execute(text(f"""
...
def api_deuda_clientes():
    ...
    try:
        vendedor_id = request.args.get('vendedor_id', type=int)
        clientes = _deuda_clientes_data(db, vendedor_id)
        total = sum(c['saldo'] for c in clientes)
        nombre_vendedor = _nombre_vendedor(db, vendedor_id)
        # Lista de vendedores para el combo del front
        vendedores = []
        try:
            for v in db.session.execute(text(
                    "SELECT id, nombre FROM vendedor WHERE activo = 1 ORDER BY nombre")).mappings().all():
...
def api_deuda_clientes_pdf():
    ...
    try:
        ...
        if nombre_vendedor:
            elems.append(Paragraph(f"Vendedor: <b>{nombre_vendedor}</b>", st_sub))
        cab = f"<b>{emisor_nombre}</b>"
        if emisor_detalles:
        ...
        if not clientes:
            ...
            elems.append(Paragraph(
                ...
                f"{'Total del vendedor' if nombre_vendedor else 'Total general'}: "
                f"<b>{_f(total_general)}</b>", st_norm))
